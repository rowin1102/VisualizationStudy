...
""" read_csv() : csv파일을 데이터프레임으로 변환한다. 별도의 옵션이 없으면 첫 번째 행은
        컬럼명으로 간주한다. 그리고 행 인덱스는 정수형으로 지정. """
df1 = pd.read_csv(file_path)
print(df1)
print('마지막 데이터', df1.loc[2, 'c3'])
print('='*30)

""" haeder 옵션 : 열의 이름으로 사용될 행의 번호를 지정한다.
        만약 첫 행부터 데이터가 있다면 None으로 지정. 이 경우 컬럼은 정수형으로 지정된다. """
...
""" index_col : 행의 인덱스로 사용할 열의 번호를 지정한다. 
        header 옵션이 없으므로 첫 행은 컬럼명으로 지정된다. """
...
""" names 옵션으로 컬럼을 지정하는 경우 데이터의 갯수보다 적으면 끝에서부터 맞춰 적용된다. 
    즉, c0이 인덱스가 되고, c1~c3까지는 데이터가 된다. """
df6 = pd.read_csv(file_path, names=['유비', '관우', '장비'])
print(df6)
print('마지막 데이터', df6.loc['2', '장비'])
print('='*30)

""" skiprows :  처음 몇 개의 행을 건너뛴 후 데이터프레임으로 변환할지 설정한다. """
